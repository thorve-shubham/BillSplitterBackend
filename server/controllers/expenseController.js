...
async function createExpense(req, res) {
  const paidByUser = await User.findOne({ userId: req.body.paidBy }).select(
    "userId firstName lastName"
  );

  const membersdb = await User.find({
    userId: { $in: req.body.members },
  ...
  let historydata = {
    ...
    message:
      req.body.userName + " Created an expense named : " + expense.expenseName,
  };

  let msg =
    req.body.userName +
    " Created an expense named : " +
    expense.expenseName +
    "<br/>" +
    "Group Name : " +
    data.groupName +
  ...
  if (
    (await history(historydata)) &&
    email.sendMail(data.emails, "Bill Splitter Update", msg)
  ) {
    return res.send(
      generateResponse(
        200,
...
async function getByGroupId(req, res) {
  const expenses = await Expense.find({ groupId: req.body.groupId }).sort({
    createdOn: -1,
  });
  if (isEmpty(expenses)) {
    return res.send(
      generateResponse(404, true, null, "No Expenses Found for this Group")
    );
  } else {
...
async function deleteExpense(req, res) {
  ...
  if (isEmpty(expense)) {
    ...
    if (
      ...
      return res.send(
        generateResponse(200, false, expense.toObject(), "Deleted Successfully")
      );
    }
  }
  //add history
...
async function updateExpense(req, res) {
  ...
  let Msg =
    expense.expenseName +
    " Expense got Updated By User : <br/><h3>" +
    req.body.userWhoModified +
    "</h3><br/>" +
    "Group Name : " +
    data.groupName +
  ...
  if (
    (await history(historydata)) &&
    email.sendMail(data.emails, "Bill Slitter Update", Msg)
  ) {
